...
def analyse_block(block): 
    ...
    front_instr_addr=None
    for instr_addr in Heads(block.start_ea, block.end_ea):  
        # instr=print_insn_mnem(instr_addr)
        instr=idc.GetDisasm(instr_addr) 
        if "cs:__afl_area_ptr" in instr:
            is_instrument=1 
            continue
        elif "cs:__afl_laf_area_ptr" in instr: 
            break
        elif is_instrument==1:
            block_id=0
            if "inc" in instr:
                block_id=get_operand_value(front_instr_addr,1) 
            else:
                block_id=get_operand_value(instr_addr,1) 
            global block_instrument_map
